...
/**
 * Liste des produits: recherche, tri, pagination côté serveur, export et
 * suppression avec confirmation. Signale visuellement les stocks faibles.
 */
@Component({
...
export class ProductList implements OnInit {
  ...
  confirmDelete(product: IProduct): void {
    const dialogRef = this.dialog.open(ConfirmDialog, {
      data: {
        title: this.translateService.instant('products.deleteConfirmTitle'),
        message: this.translateService.instant('products.deleteConfirmMessage'),
        destructive: true,
      },
    });
  ...
  exportCsv(): void {
    exportToCsv('produits', this.products(), [
      'name',
      'sku',
      'category',
      'unitPrice',
  ...
  protected statusLabel(status: ProductStatus): string {
    return status === ProductStatus.ACTIVE ? 'common.active' : 'common.inactive';
  }
  ...
  private deleteProduct(product: IProduct): void {
    this.productService.delete(product.id).subscribe(() => {
      this.notificationService.success(this.translateService.instant('products.deletedSuccess'));
      this.loadProducts();
    });
  }
}
