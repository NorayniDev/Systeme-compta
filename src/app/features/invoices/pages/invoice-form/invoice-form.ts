...
import { InvoiceLineDto } from '../../models/invoice.dto';
import {
...
/**
 * Formulaire de création/édition d'une facture: sélection du client par
 * recherche asynchrone, lignes dynamiques (FormArray) et calcul des totaux
 * HT/TVA/TTC en temps réel via `computeLineItemTotals`.
 */
@Component({
...
export class InvoiceForm implements OnInit {
  ...
  private currentStatus: InvoiceStatus = InvoiceStatus.DRAFT;
  ...
  ngOnInit(): void {
    ...
    this.form.valueChanges.pipe(startWith(this.form.getRawValue())).subscribe(() => {
      this.totals.set(computeLineItemTotals(this.lines.getRawValue() as InvoiceLineDto[]));
    });
  ...
  lineTotal(index: number): number {
    const line = this.lines.at(index).getRawValue() as InvoiceLineDto;
  ...
  submit(): void {
    ...
    const request$ = id
      ? this.invoiceService.update(id, { ...dto, status: this.currentStatus })
      : this.invoiceService.create(dto);

    request$.pipe(finalize(() => this.isSubmitting.set(false))).subscribe(() => {
      const messageKey = id ? 'invoices.updatedSuccess' : 'invoices.createdSuccess';
  ...
  private loadInvoice(id: string): void {
    ...
    this.invoiceService.getById(id).subscribe((invoice) => {
      ...
      this.clientSearchControl.setValue(invoice.clientName, { emitEvent: false });
